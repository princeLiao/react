...
var avatarUpload = function (options) {
    ...
    function handleFiles(fn) {
        ...
        img.onload = function () {
            EXIF.getData(img, function () {
                ...
                $(options.uploadBgId).css("backgroundImage", "url(\"" + imgObjectURL + "\")");
                $(options.uploadBgId).css("transform", 'translate3d(0,0,0) ' + 'scale(1,1)' + ' rotate(' + rotateAngel + 'deg) ');
            });
        }
    ...
    function createImg(fn) {
        ...
        var imgMask = document.getElementById(options.clipMask);
        var img = new Image();
        img.src = imgObjectURL
        img.onload = function () {
            //在画布上放置图像的 x 坐标位置。
            ...
            var obj={
                sx:offsetx,
                sy:offsety,
                width: biliW * imgData.scale,
                height: biliH * imgData.scale,
            }
            context.save();
            context.translate(obj.sx+obj.width/2,obj.sy+obj.height/2);
			context.rotate(rotateAngel * Math.PI / 180);
            context.translate(-(obj.sx+obj.width/2),-(obj.sy+obj.height/2));
            context.drawImage(img,obj.sx,obj.sy,obj.width,obj.height );
            context.restore();
            if (imgMask.src.length > 0) {
                context.drawImage(imgMask, 0, 0, imgMask.width, imgMask.height);
            }
            if (Object.prototype.toString.call(fn) === "[object Function]") {
                fn();
            }
    ...
    mc.on("pinchmove", function (ev) {
        ...
        imgData.tempScale = ev.scale
        $uploadBg.css("transform", 'translate3d(' + imgData.move.x + 'px,' + imgData.move.y + 'px,0) ' + 'scale(' + imgData.scale + ',' + imgData.scale + ')' + ' rotate(' + rotateAngel + 'deg) ')
    ...
    mc.on("panmove", function (ev) {
        ...
        $uploadBg.css({
            "transform": 'translate3d(' + imgData.move.x + 'px,' + imgData.move.y + 'px,0) ' + 'scale(' + imgData.scale + ',' + imgData.scale + ')' + ' rotate(' + rotateAngel + 'deg) '
        })
